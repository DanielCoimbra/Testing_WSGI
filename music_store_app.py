...
def music_store_app(environ, start_response):
    ...
    if path == "/" or path == '':  # root directory
        ...
        if len(path_sections) > 2:
            ...
            if path_sections[2] != '':
                ...
                if environ['REQUEST_METHOD'].casefold() == 'get':
                    result = template(
                        "Track Page", track_page(path_sections[2]))
                    ...
                    track_form_handler(path_sections[2], track[3:len(
                        track)-2], composer[3:len(composer)-2])
                    response_headers = [
                        ('Content-type', 'text/html'), ('Location', 'http://127.0.0.1:8000/tracks')]
        ...
        result = template(
            '404 NOT FOUND', """<pre style="font-size:18px;">                                RESOURCE NOT FOUND.</pre>""")
